 ...
 *  1) 事件源 继承了 ApplicationEvent 的类 ActionEvent
 *  2) 监听者 继承了 ApplicationListener 的类 ActionListener
 *  3) 传播者 继承了 ApplicationContextAware 的类 LoginAction, 在这个类当中的ApplicationContext 是spring自动注入的
 *  4) 在spring 容器里只需要注册传播者和监听者, 不需要注册事件源.  
 *  5) 就是说ApplicationContext 在这个 继承了 ApplicationContextAware 的类中的某个方法中将ActionEvent publish 出去
 *  然后在这个继承了 ApplicationListener 的类中就能得到这个事件.
 *  
